...
默认配置使用 `MODEL_PROVIDER=demo`，不需要 API Key，适合先确认页面、路由、健康检查和部署链路是否正常。
...
MODEL_BASE_URL=你的模型服务 Base URL
...
- 第一次部署建议保持 `MODEL_PROVIDER=demo`。
- demo 跑通后，再改成 `openai-compatible`。
